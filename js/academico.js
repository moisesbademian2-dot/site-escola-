...
App.modalNota = function () {
  ...
  if (BIMESTRES.every(b => this.bimestreFechado(b))) { Toast.warning('Todos os bimestres estão fechados para notas.'); return; }
  const body = '<form id="form-nota" novalidate><div class="form-grid">' +
    ...
    '<div class="field-group"><label>Bimestre <span class="req">*</span></label><select name="bimestre" required><option value="">Selecione...</option>' + BIMESTRES.map(b => this.bimestreFechado(b) ? '<option disabled>' + b + ' (fechado)</option>' : '<option>' + b + '</option>').join('') + '</select><div class="err">Selecione o bimestre.</div></div>' +
